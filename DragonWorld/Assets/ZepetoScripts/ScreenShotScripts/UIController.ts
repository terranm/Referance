import { Canvas, GameObject, Screen, Rect, RectTransform, Resources, WaitForSeconds, YieldInstruction, Sprite, RenderTexture } from 'UnityEngine';
import { Button, Image, RawImage, Text, Toggle } from 'UnityEngine.UI'
...
export default class UIController extends ZepetoScriptBehaviour {
    ...
    public screenShotTempPanel : Image;
    ...
    public shootDoubleScreenShotButton: Button; // 2개 찍어서 합치는 스샷
    ...
    Start() {
        ...
        this.shootDoubleScreenShotButton.onClick.AddListener(() => {
            ...
            // let rt = new RenderTexture(this.screenShot.renderTexture);
            // renderTextures.push(rt);
            
            this.screenShotTempPanel.gameObject.SetActive(true);
            this.screenShotTempPanel.transform.GetChild(cnt).GetComponent<RawImage>().texture = rt//this.screenShot.renderTexture;
            cnt++;

            if (cnt >= 4){
                this.screenShotTempPanel.gameObject.SetActive(false);
                this.screenShotTempPanel.transform.GetChild(0).GetComponent<RawImage>().texture = null;
                this.screenShotTempPanel.transform.GetChild(1).GetComponent<RawImage>().texture = null;
                this.screenShotTempPanel.transform.GetChild(2).GetComponent<RawImage>().texture = null;
                this.screenShotTempPanel.transform.GetChild(3).GetComponent<RawImage>().texture = null;
