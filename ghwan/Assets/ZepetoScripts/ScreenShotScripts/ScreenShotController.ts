...
export default class ScreenShotController extends ZepetoScriptBehaviour {
    ...
    TakeScreenShotWithBackground() {
        let glasses = GameManager.GetInstance.glassesMap.get(GameManager.GetInstance.room.SessionId);
        let balloon = GameManager.GetInstance.ballonMap.get(GameManager.GetInstance.room.SessionId);
        let wing = GameManager.GetInstance.wingMap.get(GameManager.GetInstance.room.SessionId);

        if(glasses != null && glasses.activeSelf == true
            && balloon != null && balloon.activeSelf == true
            && wing != null && wing.activeSelf == true
            && GameManager.GetInstance.onTarget == true){
                    GameManager.UI.ShowUI("20", 1);
        }
