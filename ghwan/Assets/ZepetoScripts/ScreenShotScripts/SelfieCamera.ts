...
import { Vector3, Transform, Mathf, Time, Quaternion, HideFlags, GameObject, Input, Application ,Physics, RaycastHit, Gizmos, Color, LayerMask} from 'UnityEngine'
...
export default class SelfieCamera extends ZepetoScriptBehaviour {
    ...
    layerMask = 1 << LayerMask.NameToLayer("PhotoZone");
    ...
    Spherecast(){
        if(GameManager.GetInstance.photozone == false){
            return;
        }
        let ref = $ref<RaycastHit>();
        if (Physics.SphereCast(this.transform.position, 0.5, this.transform.forward, ref, 5,this.layerMask))
        {
            let hitInfo = $unref(ref);
            //Gizmos.color = Color.red;
            //Gizmos.DrawRay(this.transform.position, Vector3.down * hitInfo.distance);

            //Gizmos.color = Color.green;
            //Gizmos.DrawWireSphere(this.transform.position + this.transform.forward * hitInfo.distance, 1);
            //console.log(hitInfo.collider.name);
            //Gizmos.color = Color.blue;
            //Gizmos.DrawSphere(hitInfo.point, 1);
            GameManager.GetInstance.onTarget = true;
        }
        else{
            GameManager.GetInstance.onTarget = false;
        }
    }
