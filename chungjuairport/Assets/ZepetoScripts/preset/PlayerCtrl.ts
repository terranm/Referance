import { Animator, Avatar, BoxCollider, Camera,  CharacterController, Collider, ForceMode, GameObject, Mathf, Quaternion, Rigidbody, RigidbodyConstraints, RuntimeAnimatorController, Time, Transform, Vector3, WaitForSeconds } from 'UnityEngine';
...
export default class PlayerCtrl extends ZepetoScriptBehaviour {
    ...
    //Game 1
    public isGame1Clear : boolean = false;
    public isGetPassport : boolean = false;
    public isGetDisembarkationCard : boolean = false;
    ...
    public Teleport(position){
        if (this.isRide){
            this.CarStop();
        }
        
        this.StartCoroutine(this.Teleport_character(position));
    ...
    *Teleport_character(position:Vector3){
        while(ZepetoPlayers.instance.LocalPlayer.zepetoPlayer.character.transform.position != position){ //new Vector3(-29,66,-123)
            yield new WaitForSeconds(0.01);
            ZepetoPlayers.instance.LocalPlayer.zepetoPlayer.character.StopMoving();
            ZepetoPlayers.instance.LocalPlayer.zepetoPlayer.character.transform.position = position // new Vector3(-29,66,-123);
        }
    }
    ...
    *ZipLine_Move(){
    ...
    public CarRide(other:GameObject){
        ...
        if(other.tag == "Car"){
            ...
            ZepetoPlayers.instance.LocalPlayer.zepetoPlayer.character.transform.position = new Vector3(-25, 55.5, -108);
